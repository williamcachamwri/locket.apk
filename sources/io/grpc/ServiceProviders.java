...
final class ServiceProviders {
    ...
    static <T> Iterable<T> getCandidatesViaHardCoded(Class<T> cls, Iterable<Class<?>> iterable) {
        ...
        for (Class<?> createForHardCoded : iterable) {
            T createForHardCoded2 = createForHardCoded(cls, createForHardCoded);
            if (createForHardCoded2 != null) {
                arrayList.add(createForHardCoded2);
            }
        }
        return arrayList;
    }
