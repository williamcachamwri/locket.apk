...
public class AdaptiveStreamBuffer {
    ...
    private int resize(int i) {
        ...
        if (!this.adaptiveMode || ((long) max) >= maxMemory) {
            SentryLogcatAdapter.w(TAG, "Turning off adaptive buffer resizing to conserve memory.");
        } else {
            try {
                ...
                SentryLogcatAdapter.w(TAG, "Turning off adaptive buffer resizing due to low memory.");
