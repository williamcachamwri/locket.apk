        ...
        if (stream != null) {
            ...
            if (!file.exists()) {
                if (this.mResumeOffset > 0) {
                    ...
                    SentryLogcatAdapter.w(TAG, "unable to create file:" + file.getAbsolutePath());
                }
            }
            boolean z = true;
            if (this.mResumeOffset > 0) {
                Log.d(TAG, "Resuming download file " + file.getAbsolutePath() + " at " + this.mResumeOffset);
                fileOutputStream = SentryFileOutputStream.Factory.create(new FileOutputStream(file, true), file, true);
            } else {
                ...
                if (z2) {
                    ...
                    if (TextUtils.isEmpty(resultString) || (str = this.mETagVerification) == null || str.equals(resultString)) {
                        ...
                        try {
                            ...
                            SentryLogcatAdapter.e(TAG, "Exception occurred during file write.  Aborting.", e);
                            this.mException = e;
                        }
                    } else {
                        SentryLogcatAdapter.w(TAG, "The file at the server has changed.  Restarting from the beginning.");
