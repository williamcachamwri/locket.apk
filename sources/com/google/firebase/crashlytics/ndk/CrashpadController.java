...
public class CrashpadController {
    ...
    private CrashlyticsReport.ApplicationExitInfo getRelevantApplicationExitInfo(long j, List<ApplicationExitInfo> list) {
        ...
        for (ApplicationExitInfo next : list) {
            if (next.getReason() == 5 && next.getTimestamp() >= j) {
                arrayList.add(next);
            }
        }
    ...
    private static void writeTextFile(File file, String str) {
        ...
        try {
            BufferedWriter bufferedWriter2 = new BufferedWriter(new OutputStreamWriter(SentryFileOutputStream.Factory.create(new FileOutputStream(file), file), UTF_8));
            try {
