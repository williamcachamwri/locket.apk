...
class Metadata {
    ...
    public synchronized int getIidImplementation() {
        ...
        if (packageManager.checkPermission(GMSCORE_SEND_PERMISSION, "com.google.android.gms") == -1) {
            SentryLogcatAdapter.e(Constants.TAG, "Google Play services missing or without correct permission.");
            return 0;
        }
        ...
        if (queryBroadcastReceivers == null || queryBroadcastReceivers.size() <= 0) {
            SentryLogcatAdapter.w(Constants.TAG, "Failed to resolve IID implementation package, falling back");
            if (PlatformVersion.isAtLeastO()) {
    ...
    private PackageInfo getPackageInfo(String str) {
        try {
            ...
            SentryLogcatAdapter.w(Constants.TAG, "Failed to find package " + e);
            return null;
        }
    }
}
