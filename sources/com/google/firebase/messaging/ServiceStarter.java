...
public class ServiceStarter {
    ...
    private int doStartService(Context context, Intent intent) {
        ...
        try {
            ...
            SentryLogcatAdapter.e(Constants.TAG, "Error while delivering the message: ServiceIntent not found.");
            return ERROR_NOT_FOUND;
        } catch (SecurityException e) {
            SentryLogcatAdapter.e(Constants.TAG, "Error while delivering the message to the serviceIntent", e);
            return 401;
        } catch (IllegalStateException e2) {
            SentryLogcatAdapter.e(Constants.TAG, "Failed to start service while in background: " + e2);
            return 402;
        }
    }
    ...
    private synchronized String resolveServiceClassName(Context context, Intent intent) {
        ...
        if (resolveService != null) {
            if (resolveService.serviceInfo != null) {
                ...
                SentryLogcatAdapter.e(Constants.TAG, "Error resolving target intent service, skipping classname enforcement. Resolved service was: " + serviceInfo.packageName + "/" + serviceInfo.name);
                return null;
            }
        }
        SentryLogcatAdapter.e(Constants.TAG, "Failed to resolve target intent service, skipping classname enforcement");
        return null;
    }
