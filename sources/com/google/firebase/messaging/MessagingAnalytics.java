...
public class MessagingAnalytics {
    ...
    private static void setUserPropertyIfRequired(Bundle bundle) {
        if (bundle != null) {
            if ("1".equals(bundle.getString(Constants.AnalyticsKeys.TRACK_CONVERSIONS))) {
                ...
                SentryLogcatAdapter.w(Constants.TAG, "Unable to set user property for conversion tracking:  analytics library is missing");
        ...
        try {
            ...
            if (messageTime != null) {
                try {
                    ...
                    SentryLogcatAdapter.w(Constants.TAG, "Error while parsing timestamp in GCM event", e);
                }
            }
            ...
            if (useDeviceTime != null) {
                try {
                    ...
                    SentryLogcatAdapter.w(Constants.TAG, "Error while parsing use_device_time in GCM event", e2);
                }
            }
            ...
            if (analyticsConnector != null) {
                ...
                SentryLogcatAdapter.w(Constants.TAG, "Unable to log event: analytics library is missing");
            }
        } catch (IllegalStateException unused) {
            SentryLogcatAdapter.e(Constants.TAG, "Default FirebaseApp has not been initialized. Skip logging event to GA.");
        }
    }

    private static void logToFirelog(MessagingClientEvent.Event event, Intent intent, TransportFactory transportFactory) {
        if (transportFactory == null) {
            SentryLogcatAdapter.e(Constants.TAG, "TransportFactory is null. Skip exporting message delivery metrics to Big Query");
            return;
        }
        ...
        if (eventToProto != null) {
            try {
                ...
                SentryLogcatAdapter.w(Constants.TAG, "Failed to send big query analytics payload.", e);
            }
        }
    }
    ...
    static int getTtl(Bundle bundle) {
        ...
        try {
            ...
            SentryLogcatAdapter.w(Constants.TAG, "Invalid TTL: " + obj);
            return 0;
        }
    }
    ...
    static long getProjectNumber(Bundle bundle) {
        if (bundle.containsKey(Constants.MessagePayloadKeys.SENDER_ID)) {
            try {
                ...
                SentryLogcatAdapter.w(Constants.TAG, "error parsing project number", e);
            }
        }
        ...
        if (gcmSenderId != null) {
            try {
                ...
                SentryLogcatAdapter.w(Constants.TAG, "error parsing sender ID", e2);
            }
        }
        ...
        if (!applicationId.startsWith("1:")) {
            try {
                ...
                SentryLogcatAdapter.w(Constants.TAG, "error parsing app ID", e3);
                return 0;
            }
        } else {
            ...
            try {
                ...
                SentryLogcatAdapter.w(Constants.TAG, "error parsing app ID", e4);
                return 0;
            }
        }
    }
