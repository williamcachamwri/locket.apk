...
class SyncTask implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            SentryLogcatAdapter.e(Constants.TAG, "Topic sync or token retrieval failed on hard failure exceptions: " + e.getMessage() + ". Won't retry the operation.");
            this.firebaseMessaging.setSyncScheduledOrRunning(false);
            if (!ServiceStarter.getInstance().hasWakeLockPermission(getContext())) {
            }
        } catch (Throwable th) {
    ...
    public boolean maybeRefreshToken() throws IOException {
        try {
            ...
            if (GmsRpc.isErrorMessageForRetryableError(e.getMessage())) {
                SentryLogcatAdapter.w(Constants.TAG, "Token retrieval failed: " + e.getMessage() + ". Will retry token retrieval");
                return false;
