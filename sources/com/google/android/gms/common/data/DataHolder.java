        ...
        while (i2 < size) {
            try {
                if (!cursorWindow.allocRow()) {
                    ...
                    arrayList.add(cursorWindow);
                    if (!cursorWindow.allocRow()) {
                        SentryLogcatAdapter.e("DataHolder", "Unable to allocate row to hold data.");
    ...
    public final void finalize() throws Throwable {
        try {
            if (this.zak && this.zah.length > 0 && !isClosed()) {
                close();
                String obj = toString();
                SentryLogcatAdapter.e("DataBuffer", "Internal data leak within a DataBuffer object detected!  Be sure to explicitly call release() on all DataBuffer extending objects when you are done with them. (internal object: " + obj + ")");
            }
        } finally {
            super.finalize();
        }
