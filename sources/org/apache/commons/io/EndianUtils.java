...
public class EndianUtils {
    ...
    public static long readSwappedUnsignedInteger(byte[] bArr, int i) {
        return (((long) (bArr[i + 3] & 255)) << 24) + (((long) (((bArr[i + 0] & 255) << 0) + ((bArr[i + 1] & 255) << 8) + ((bArr[i + 2] & 255) << 16))) & MuxerUtil.UNSIGNED_INT_MAX_VALUE);
    }
    ...
    public static long readSwappedUnsignedInteger(InputStream inputStream) throws IOException {
        return (((long) (read(inputStream) & 255)) << 24) + (((long) (((read(inputStream) & 255) << 0) + ((read(inputStream) & 255) << 8) + ((read(inputStream) & 255) << 16))) & MuxerUtil.UNSIGNED_INT_MAX_VALUE);
    }
